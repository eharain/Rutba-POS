 ...
 *     Users with the 'auth' app-access bypass owner filtering
 *     (they are admins).
 *
...
module.exports = (config, { strapi }) => {
  ...
  async function getUserAppAccess(userId) {
    ...
    const keys = (user?.app_accesses || []).map(a => a.key);
    accessCache.set(userId, keys);
  ...
  return async (ctx, next) => {
    ...
    const userKeys = await getUserAppAccess(user.id);

    // Admin bypass — users with 'auth' pass everything
    const isAdmin = userKeys.includes('auth');

    if (!isAdmin && !hasAny(userKeys, required)) {
    ...
    const hasOwner = model?.attributes?.owners &&
